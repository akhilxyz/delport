import React, { useLayoutEffect, useRef, useState } from "react";
import { useFormik } from "formik";
import { driver_application_schema } from "@utils/validation-schema";
import ErrorMsg from "./error-msg";
import NiceSelect from "@ui/niceSelect";
import Link from "next/link";
import axios from "axios";

const ContactForm = ({ iR }) => {

  const handleSelectChange = (value) => {

    if (value.value === "Select Subject") {
      setFieldValue('selectedValue', '');
    } else {
      setFieldValue('selectedValue', value.value);
    }
  };


  const inputRef = useRef(null);


  useLayoutEffect(() => {
    iR(inputRef);
  }, []);



  const { handleChange, handleSubmit, handleBlur, errors, values, touched, setFieldValue, isSubmitting } = useFormik({
    initialValues: {
      firstName: "",
      lastName: "",
      email: "",
      phone: "",
      selectedValue: "",
      driverLicense: "",
      exp: "",
      msg: "",
    },

    validationSchema: driver_application_schema,


    onSubmit: async (values, { resetForm, setSubmitting }) => {

      setSubmitting(true);

      try {
        const response = axios.post('/api/send-email', {
          to: 'hiring@goldenmileinc.com',
          subject: 'Golden Mile New Driver Application',
          data: values,
          type:'DRIVERAPPLICATION'
        })
        alert('Form Submitted successfully!');
        resetForm();
      }
      catch (error) {
        console.error(error);
      }
      finally {
        setSubmitting(false);
      }
    },


  });

  return (
    <form id="driver-application-form" onSubmit={handleSubmit}>
      <div className="row">
        <div className="col-xxl-6 col-xl-6 col-lg-6">
          <div className="single-input-field">
            <input
              ref={inputRef}
              name="firstName"
              value={values.firstName}
              onChange={handleChange}
              onBlur={handleBlur}
              type="text"
              placeholder="First Name"
              id="firstName"
            />
            <i className="fas fa-user"></i>
            {touched.firstName && errors.firstName && <ErrorMsg error={errors.firstName} />}
          </div>
        </div>
        <div className="col-xxl-6 col-xl-6 col-lg-6">
          <div className="single-input-field">
            <input
              name="lastName"
              value={values.lastName}
              onChange={handleChange}
              onBlur={handleBlur}
              type="text"
              placeholder="Last Name"
              id="lastName"
            />
            <i className="fas fa-user"></i>
            {touched.lastName && errors.lastName && <ErrorMsg error={errors.lastName} />}
          </div>
        </div>
        <div className="col-xxl-6 col-xl-6 col-lg-6">
          <div className="single-input-field">
            <input
              name="email"
              value={values.email}
              onChange={handleChange}
              onBlur={handleBlur}
              type="email"
              placeholder="Email Address"
              id="email"
            />
            <i className="fas fa-envelope"></i>
            {touched.email && errors.email && <ErrorMsg error={errors.email} />}
          </div>
        </div>
        <div className="col-xxl-6 col-xl-6 col-lg-6">
          <div className="single-input-field">
            <input
              name="phone"
              value={values.phone}
              onChange={handleChange}
              onBlur={handleBlur}
              type="text"
              placeholder="Phone"
              id="phone"
            />
            <i className="fas fa-phone-alt"></i>
            {touched.phone && errors.phone && <ErrorMsg error={errors.phone} />}
          </div>
        </div>


        <div className="col-xxl-6 col-xl-6 col-lg-6">
          <div className="single-input-field">
            <input
              style={{ width: '100%' }}
              name="driverLicense"
              value={values.driverLicense}
              onChange={handleChange}
              onBlur={handleBlur}
              type="text"
              placeholder="Driver license number"
              id="driverLicense"
            />
            <i className="fas fa-user"></i>
            {touched.driverLicense && errors.driverLicense && <ErrorMsg error={errors.driverLicense} />}
          </div>
        </div>


        <div className="col-xxl-6 col-xl-6 col-lg-6">
          <div className="single-input-field">
            <input
              name="exp"
              value={values.exp}
              onChange={handleChange}
              onBlur={handleBlur}
              min={0}
              type="number"
              placeholder="Years of Exprience"
              id="exp"
            />
            <i className="fas fa-user"></i>
            {touched.exp && errors.exp && <ErrorMsg error={errors.exp} />}
          </div>
        </div>

        <div className="col-xxl-6 col-xl-6 col-lg-6">
          <div className="single-input-field select">
            <NiceSelect
              options={[
                { value: "Applying For", text: "Applying For" },
                { value: "Local", text: "Local" },
                { value: "Regional", text: "Regional" },
                { value: "OTR", text: "OTR" },
              ]}
              defaultCurrent={0}
              name="selectSubject"
              value={values.selectedValue}
              onChange={handleSelectChange}
            />
            {touched.selectedValue && errors.selectedValue && (
              <ErrorMsg error={errors.selectedValue} />
            )}
          </div>
        </div>

        <div className="col-xxl-12 col-xl-12 col-lg-12">
          <div className="single-input-field textarea">
            <textarea
              name="msg"
              rows="10"
              cols="10"
              value={values.msg}
              onChange={handleChange}
              onBlur={handleBlur}
              id="msg"
              placeholder="Work History"
            ></textarea>
            <i className="fas fa-edit"></i>
            {touched.msg && errors.msg && <ErrorMsg error={errors.msg} />}
          </div>
        </div>
        <p style={{ fontSize: "12px" }}>
          On Submitting this form, you agree to Golden Mile Enterprise&apos;s Terms of Use and Privacy Policy. You consent to receive phone calls and SMS messages from Golden Mile Enterprise to provide updates and information regarding your business with Golden Mile Enterprise. Message frequency may vary. Message & data rates may apply. Reply STOP to opt-out of further messaging. Reply HELP for more information. See our <Link href="/privacy-policy" style={{ color: "blue" }}>Privacy Policy</Link>.
        </p>
        <div className="col-xxl-12 col-xl-12">
          <button type="submit" disabled={isSubmitting} className="fill-btn clip-btn ">Submit Application</button>
        </div>
      </div>
    </form>
  );
};

export default ContactForm;
